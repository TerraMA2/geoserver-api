...
exports.get = async (workspaceName, styleName = '', geoserverBasePath) => {
    const url = `rest/workspaces/${ workspaceName }/styles${ styleName ? '/' + styleName : '' }`;
    const method = 'get';
...
exports.create = async (data, workspaceName, geoserverBasePath) => {
    ...
    const url = `rest/workspaces/${ workspaceName }/styles`;
    const method = 'POST';
...
exports.update = async (data, workspaceName, styleName, geoserverBasePath) => {
    const url = `rest/workspaces/${ workspaceName }/styles/${ styleName }`;
    const method = 'put';
    ...
    return await request({
        ...
        .then(async res => {
            ...
            if (updateResponse.status !== 200 && updateResponse.status !== 201) {
                return response(updateResponse.status, `Style ${ styleName } not uploaded`);
            }
            return response(updateResponse.status, `Style ${ styleName } uploaded`);
        })
        .catch((error) => handleErrors(error, `Couldn't create style ${ styleName }`));
}
